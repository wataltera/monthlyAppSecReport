...
# Build filename: GroupNameMendReportMMM dd yy.xlsx
$filename = "$whichBU" + "MendReport_" + $timestamp + ".xlsx"
...
try {
    ...
    elseif (Test-Path $jsonTemplatePath) {
        ...
        Write-Host "Using template config: $jsonTemplatePath (consider creating ProductGroups.local.json with real tokens)" -ForegroundColor Yellow
    }
    else {
    ...
    # Select the group requested on the command line
    if ($groups.$whichBU) {
        $ProductToken = $groups.$whichBU
    }
    else {
        throw "Group '$whichBU' not found in $configPath. Available groups: $($groups.PSObject.Properties.Name -join ', ')"
    }

    # Convert it to a true hash table to be compatible with the original program
    $ProductToken = @{}
    $groups.$whichBU.PSObject.Properties | ForEach-Object {
    ...
    $ProductToken.GetEnumerator() | ForEach-Object {
        ...
        Write-Host "Collecting Data of $whichBU $ProductName ..." -ForegroundColor Green
        ...
        foreach($project in $projects)
        {
            ...
            Write-Output "`t`tworking on $whichBU $projectName"
            Write-Host "`t`tworking on $whichBU $projectName"
            #Get "Last Scan Date"
            ...
            # Dump vulnerabilities found in the scan
            ...
            $details = [ordered]@{            
                ...
                "Latest Scan Date (dd/MM/yyyy)"  = $Date 
            ...
            <# Now populate the Artifacts and Scans tables.  We have all variables in the $details
            Get ID of an Artifact record for $ProductName, $projectName and $whichBU 
            Create it if it does not exist.  Put ID into $artifactID
            $artifactID = Get-ArtifactID -ProductName $ProductName -ProjectName $projectName -BusinessUnit $whichBU
            Check if there is already a scan for $artifactID on $Date, ScanType=SCA, ScanTool=Mend
            If there is, continue.
            If not, see if the scan with maximum scan ID where ScanType-SCA and ScanTool=Mend
            has the same vulnerability counts as this scan.  
                If it does, update ScanDate to $Date and increment ScanRepeatCount by 1.
                If not, insert a new scan row with the data from $details and ScanTool=Mend,
                ScanType=SCA, ScanDateTime=$Date, ScanRepeatCount=1, and vulnerability counts
